...
class LegacyInletV5Controller(
  ...
  def lineage(@RequestBody body: Array[Byte]): Future[Void] = {
    ...
    for {
      linked <- batchLinker(processed)
       _ <- lineageWriter.store(linked)
         .recover({ case e: DuplicateKeyException =>
           log.warn("Ignored DuplicateKeyException to tolerate duplicate lineage insert to MongoDB.", e)
  ...
  def progress(@RequestBody body: Array[Byte]): Future[Void] = {
