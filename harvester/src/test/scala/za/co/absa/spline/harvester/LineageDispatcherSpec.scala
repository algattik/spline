...
class LineageDispatcherSpec extends FunSuite with Matchers with LineageFixture {

  test("testSend with retries") {
    var captured = Array[Byte]()
    var retriesUntilSuccess = 3
    def attemptSendSuccessMock(url: String, body: Array[Byte]): Boolean = {
      captured = body
      retriesUntilSuccess = retriesUntilSuccess - 1
      retriesUntilSuccess == 0
    }
    val expectedLineage = fiveOpsLineage()
    new LineageDispatcher(3, "http://localhost/dummy", attemptSendSuccessMock)
      .send(expectedLineage)
    val actualLineage = grater[DataLineage].fromBSON(captured)
