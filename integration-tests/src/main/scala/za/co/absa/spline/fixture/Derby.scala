...
trait DerbyDatabaseFixture extends BeforeAndAfter{
  ...
  val connectionString = "jdbc:derby:memory:" + dbName + ";create=true"

  val connection = {
    val tempPath = TempDirectory("derbyUnitTest", "database", false).deleteOnExit().path
  ...
  def createTable(table: String): ResultSet = {
    execute("Create table " + table + " (id int, name varchar(30))")
  }

  def dropTable(table: String): ResultSet = {
